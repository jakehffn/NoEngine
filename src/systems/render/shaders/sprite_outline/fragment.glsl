...
void main() {
    ...
    bool near_color = false;
    
    if (pixel_x > 1 && pixel_y > 0 && pixel_x < texture_data.z && pixel_y < texture_data.w) {
        ...
        if (current_color.a > 0) {
            ...
            if (pixel_x < texture_data.z - 1) {
                near_color = near_color || (sampleTexture(pixel_x+1, pixel_y).a > 0);
            } 
            if (pixel_y < texture_data.w) {
                near_color = near_color || (sampleTexture(pixel_x, pixel_y+1).a > 0);
            } 
            if (pixel_x > 1) {
                near_color = near_color || (sampleTexture(pixel_x-1, pixel_y).a > 0);
            }
        ...
        if (pixel_x < 1 && pixel_y > 0 && pixel_y < texture_data.w) {
            near_color = near_color || (sampleTexture(pixel_x+1, pixel_y).a > 0);
        } 
        if (pixel_y < 0 && pixel_x > 0 && pixel_x < texture_data.z - 1) {
            near_color = near_color || (sampleTexture(pixel_x, pixel_y+1).a > 0);
        } 
        if (pixel_x > texture_data.z && pixel_y > 0 && pixel_y < texture_data.w) {
            near_color = near_color || (sampleTexture(pixel_x-1, pixel_y).a > 0);
        }
        if (pixel_y > texture_data.w && pixel_x > 0 && pixel_x < texture_data.z - 1) {
            near_color = near_color || (sampleTexture(pixel_x, pixel_y-1).a > 0);
        }
        if (near_color) {
            color = vec4(1, 1, 1, 1);
