...
public:
    ...
    void beginTileSet(entt::registry& registry, entt::entity entity, std::string sprite_sheet_name);
...
private:
    ...
    void beginTileSetAseprite(entt::registry& registry, entt::entity entity, std::string sprite_sheet_name);
    void endTileSetAseprite();

    rapidjson::Document readJSON(std::string json_path);
    SpriteSheet& initNewSpriteSheet(const rapidjson::Value& document, std::string sprite_sheet_name);
    std::tuple<std::string, DIRECTION, int> parseFrameName(std::string frame_name);
